...
def parse_args():
    ...
    parser.add_argument("--override_lr", type=float, default=None, help="Override learning rate when resuming (optional)")
    parser.add_argument("--override_mask_ratio", type=float, default=None, help="Override mask ratio when resuming (optional)")
    parser.add_argument("--resume", type=str, default=None, help="Path to checkpoint to resume from")
    parser.add_argument("--start_epoch", type=int, default=1, help="Starting epoch (automatically set when resuming)")
...
def train_epoch(model, dataloader, optimizer, scheduler, device, epoch, args, distributed=False):
    ...
    for i, batch in enumerate(pbar):
        ...
        total_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1e6)  # max_norm=1e6 ≈ no clipping
        print(f"Pre-clip gradient norm: {total_norm:.4f}") 
        # Gradient clipping
...
def distributed_main(rank, world_size, args):
    ...
    print(f"Data loaders recreated with new mask ratio: {args.mask_ratio}")
    ...
    if args.resume and rank == 0:
        start_epoch, best_val_loss = load_checkpoint(
            args.resume, model.module, optimizer, scheduler, device, args
        )
        
        # If LR was overridden, create new scheduler
        if args.override_lr is not None:
            scheduler = get_lr_scheduler(optimizer, args, len(train_loader))

    # Broadcast values to all processes
    start_epoch = torch.tensor(start_epoch).cuda()
    best_val_loss = torch.tensor(best_val_loss).cuda()
    dist.broadcast(start_epoch, 0)
    dist.broadcast(best_val_loss, 0)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        # Save checkpoints only on rank 0 - THIS IS THE KEY FIX
        if rank == 0:
            ...
            # Save periodic checkpoints (every 10 epochs or at the end)
...
def load_checkpoint(checkpoint_path, model, optimizer, scheduler, device, args):
    ...
    if args.override_lr is not None:
        print(f"Overriding learning rate: {checkpoint['args'].get('lr', 'unknown')} -> {args.override_lr}")
        # Update optimizer learning rate
        for param_group in optimizer.param_groups:
            param_group['lr'] = args.override_lr
        # Don't load scheduler state if LR is overridden - create fresh scheduler
        print("Creating new LR scheduler due to learning rate override")
    else:
        # Load optimizer and scheduler states normally
        optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        if 'scheduler_state_dict' in checkpoint:
            scheduler.load_state_dict(checkpoint['scheduler_state_dict'])
    
    # Check for mask ratio override
    original_mask_ratio = checkpoint['args'].get('mask_ratio', 'unknown')
    if args.override_mask_ratio is not None:
        print(f"Overriding mask ratio: {original_mask_ratio} -> {args.override_mask_ratio}")
        print("WARNING: Mask ratio change will affect data loading. Make sure this is intended.")
    else:
        # Use original mask ratio if not overridden
        if args.mask_ratio != original_mask_ratio:
            print(f"Using original checkpoint mask ratio: {original_mask_ratio}")
            args.mask_ratio = original_mask_ratio
...
def main():
    ...
    if world_size > 1:
        ...
        # Exit after distributed training - don't continue to single GPU code
        return
    else:
        print("Single GPU training...")
    
    # Set up logging (only for single GPU case)
    logging.basicConfig(
        level=logging.INFO,
    ...
    if args.wandb:
        try:
            ...
            # If resuming, try to resume the W&B run
            resume_mode = "allow" if args.resume else False
    ...
    # Create data loaders (this will use the potentially overridden mask_ratio)
    train_loader, val_loader = split_and_combine_dataloaders(
        cosmic_data_path=args.cosmic_file,
        marley_data_path=args.marley_file, 
        pos_encoding_path=args.pos_encoding_file,
        ...
        train_split=args.train_split,
        seed=args.seed,
        mask_ratio=args.mask_ratio,  # This will be the overridden value if specified
        max_waveforms=max_waveforms,
        distributed=False  # Set to False for single GPU training
    )

    if args.resume and args.override_mask_ratio is not None:
       logger.info(f"Data loaders recreated with new mask ratio: {args.mask_ratio}")
    ...
    if args.resume:
        ...
        # If LR was overridden, create a new scheduler with the new LR
        if args.override_lr is not None:
            scheduler = get_lr_scheduler(optimizer, args, len(train_loader))
            print(f"Created new scheduler with LR: {args.override_lr}")
