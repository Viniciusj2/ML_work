...
def calculate_metrics(predictions, labels):
    ...
    return {
        ...
        'recall': recall,
        'f1': f1,
        'precision_cosmic': precision_per_class[0] if len(precision_per_class) > 0 else 0.0,
        'precision_marley': precision_per_class[1] if len(precision_per_class) > 1 else 0.0,
        'recall_cosmic': recall_per_class[0] if len(recall_per_class) > 0 else 0.0,
        'recall_marley': recall_per_class[1] if len(recall_per_class) > 1 else 0.0,
        'f1_cosmic': f1_per_class[0] if len(f1_per_class) > 0 else 0.0,
        'f1_marley': f1_per_class[1] if len(f1_per_class) > 1 else 0.0,
    }
...
def train_classifier_distributed(
    ...
    for epoch in epoch_pbar:
        ...
        for batch_idx, batch in enumerate(train_pbar):
            ...
            # Store predictions and labels for metrics (only on rank 0)
            if rank == 0:
            ...
            if rank == 0:
                ...
                if batch_idx % 50 == 0:
                    batch_metrics = calculate_metrics(predictions.cpu().numpy(), labels.cpu().numpy())
                    wandb.log({
                        "batch_train_loss": loss.item(),
                        "batch_train_acc": (predictions == labels).float().mean().item(),
                        "batch_train_precision": batch_metrics['precision'],
                        "batch_train_recall": batch_metrics['recall'],
                        "batch_train_f1": batch_metrics['f1'],
                        "epoch": epoch,
                        "batch": batch_idx
                    })
        ...
        # Calculate training metrics (only on rank 0)
        if rank == 0:
            train_metrics = calculate_metrics(train_all_predictions, train_all_labels)
        ...
        # Store predictions for metrics calculation (only on rank 0)
        if rank == 0:
            all_predictions = []
            all_labels = []
        ...
        # Calculate validation metrics (only on rank 0)
        if rank == 0:
            val_metrics = calculate_metrics(all_predictions, all_labels)
        ...
        if rank == 0:
            ...
            epoch_metrics = {
                ...
                "train_precision_cosmic": train_metrics['precision_cosmic'],
                "train_precision_marley": train_metrics['precision_marley'],
                "train_recall_cosmic": train_metrics['recall_cosmic'],
                "train_recall_marley": train_metrics['recall_marley'],
                "train_f1_cosmic": train_metrics['f1_cosmic'],
                "train_f1_marley": train_metrics['f1_marley'],
                "val_loss": avg_val_loss,
                "val_acc": val_acc,
                "val_precision": val_metrics['precision'],
                "val_recall": val_metrics['recall'],
                "val_f1": val_metrics['f1'],
                "val_precision_cosmic": val_metrics['precision_cosmic'],
                "val_precision_marley": val_metrics['precision_marley'],
                "val_recall_cosmic": val_metrics['recall_cosmic'],
                "val_recall_marley": val_metrics['recall_marley'],
                "val_f1_cosmic": val_metrics['f1_cosmic'],
                "val_f1_marley": val_metrics['f1_marley'],
            ...
            # Save best model (using F1 score as additional criterion)
            save_model = False
            if val_acc > best_val_acc:
                best_val_acc = val_acc
                save_model = True
                save_reason = f"accuracy: {best_val_acc:.4f}"
            
            if val_metrics['f1'] > best_val_f1:
                best_val_f1 = val_metrics['f1']
                save_model = True
                save_reason = f"F1 score: {best_val_f1:.4f}"
            ...
            if save_model:
                ...
                tqdm.write(f"  🎉 New best {save_reason}")
                
                # Save model artifact to W&B
                model_artifact = wandb.Artifact(
                    name=f"best-model-{experiment_name}",
                    type="model",
                    description=f"Best model checkpoint at epoch {epoch+1} with {save_reason}"
                )
    ...
    if rank == 0:
        ...
        wandb.log({
            ...
            "precision_curves": wandb.plot.line_series(
                xs=epochs_range,
                ys=[train_precisions, val_precisions],
                keys=["Train Precision", "Val Precision"],
                title="Training and Validation Precision",
                xname="Epoch"
            ),
            "recall_curves": wandb.plot.line_series(
                xs=epochs_range,
                ys=[train_recalls, val_recalls],
                keys=["Train Recall", "Val Recall"],
                title="Training and Validation Recall",
                xname="Epoch"
            ),
            "f1_curves": wandb.plot.line_series(
                xs=epochs_range,
                ys=[train_f1s, val_f1s],
                keys=["Train F1", "Val F1"],
